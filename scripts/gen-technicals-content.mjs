// Build-time generator for the "Technical Analysis" course content.
// Renders every chapter's markdown into a single JSON module
// (lib/technicalsContentData.json) so the Next pages need NO filesystem access at
...
// public/technicals/images/, a labelled placeholder is rendered that tells the author
...
import { CHAPTERS } from "../lib/technicalsCurriculum.js";

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");

// Optional --locale=xx flag (or LOCALE=xx env var) generates a translated
// content JSON from content/technicals/md-<locale>/ instead of the English
// content/technicals/md/, writing to lib/content-i18n/technicals/<locale>.js
// instead of lib/technicalsContentData.json. English output/behavior is
...
const MD_DIR = LOCALE === "en"
  ? path.join(ROOT, "content", "technicals", "md")
  : path.join(ROOT, "content", "technicals", `md-${LOCALE}`);
const IMG_DIR = path.join(ROOT, "public", "technicals", "images");
const DATA_OUT = LOCALE === "en"
  ? path.join(ROOT, "lib", "technicalsContentData.json")
  : path.join(ROOT, "lib", "content-i18n", "technicals", `${LOCALE}.js`);
...
function renderImage(spec) {
  ...
  const src = `/technicals/images/${file}`;
...
<code class="shot-ph-path">public/technicals/images/${esc(file)}</code>
</div>
${cap}</figure>`;
}
