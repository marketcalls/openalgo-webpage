// Build-time generator for the "Options Basics" course content.
// Renders every chapter's markdown into a single JSON module
// (lib/optionsBasicsContentData.json) so the Next pages need NO filesystem access at
...
// public/options-basics/images/, a labelled placeholder is rendered that tells the author
...
import { CHAPTERS } from "../lib/optionsBasicsCurriculum.js";

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");

// Optional --locale=xx flag (or LOCALE=xx env var) generates a translated
// content JSON from content/options-basics/md-<locale>/ instead of the English
// content/options-basics/md/, writing to lib/content-i18n/options-basics/<locale>.js
// instead of lib/optionsBasicsContentData.json. English output/behavior is
...
const MD_DIR = LOCALE === "en"
  ? path.join(ROOT, "content", "options-basics", "md")
  : path.join(ROOT, "content", "options-basics", `md-${LOCALE}`);
const IMG_DIR = path.join(ROOT, "public", "options-basics", "images");
const DATA_OUT = LOCALE === "en"
  ? path.join(ROOT, "lib", "optionsBasicsContentData.json")
  : path.join(ROOT, "lib", "content-i18n", "options-basics", `${LOCALE}.js`);
...
function renderImage(spec) {
  ...
  const src = `/options-basics/images/${file}`;
...
<code class="shot-ph-path">public/options-basics/images/${esc(file)}</code>
</div>
${cap}</figure>`;
}
