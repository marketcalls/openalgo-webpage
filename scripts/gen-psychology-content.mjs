// Build-time generator for the "Trading Psychology & Risk Playbooks" course content.
// Renders every chapter's markdown into a single JSON module
// (lib/psychologyContentData.json) so the Next pages need NO filesystem access at
// runtime - critical on Cloudflare Workers where process.cwd() is not the bundle.
//
// Content is prose + tables + inline SVG. Optional raster images are referenced
// with an {{image: file | caption | kind}} directive; when the file is not yet
// present under public/trading-psychology/images/, a labelled placeholder is shown.
//
...
import { CHAPTERS } from "../lib/psychologyCurriculum.js";

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");

// Optional --locale=xx flag (or LOCALE=xx env var) generates a translated
// content JSON from content/trading-psychology/md-<locale>/ instead of the English
// content/trading-psychology/md/, writing to lib/content-i18n/trading-psychology/<locale>.js
// instead of lib/psychologyContentData.json. English output/behavior is
...
const MD_DIR = LOCALE === "en"
  ? path.join(ROOT, "content", "trading-psychology", "md")
  : path.join(ROOT, "content", "trading-psychology", `md-${LOCALE}`);
const IMG_DIR = path.join(ROOT, "public", "trading-psychology", "images");
const DATA_OUT = LOCALE === "en"
  ? path.join(ROOT, "lib", "psychologyContentData.json")
  : path.join(ROOT, "lib", "content-i18n", "trading-psychology", `${LOCALE}.js`);
...
function renderImage(spec) {
  ...
  const src = `/trading-psychology/images/${file}`;
...
<code class="shot-ph-path">public/trading-psychology/images/${esc(file)}</code>
</div>
${cap}</figure>`;
}

function renderCallout(type, innerMd) {
  const labels = { tip: "Tip", warn: "Heads up", info: "Note", key: "Key idea", fact: "Did you know", example: "Real example", mistake: "Common mistake", check: "Self-check" };
