// Build-time generator for the "Stock Market Basics" course content.
// Renders every chapter's markdown into a single JSON module
// (lib/stocksContentData.json) so the Next pages need NO filesystem access at
...
// public/stocks/images/, a labelled placeholder is rendered that tells the author
...
import { CHAPTERS } from "../lib/stocksCurriculum.js";

// Optional --locale=xx flag (or LOCALE=xx env var) generates a translated
// content JSON from content/stocks/md-<locale>/ instead of the English
// content/stocks/md/, writing to lib/content-i18n/stocks/<locale>.json
// instead of lib/stocksContentData.json. English output/behavior is
...
const MD_DIR = LOCALE === "en"
  ? path.join(ROOT, "content", "stocks", "md")
  : path.join(ROOT, "content", "stocks", `md-${LOCALE}`);
const IMG_DIR = path.join(ROOT, "public", "stocks", "images");
const DATA_OUT = LOCALE === "en"
  ? path.join(ROOT, "lib", "stocksContentData.json")
  : path.join(ROOT, "lib", "content-i18n", "stocks", `${LOCALE}.js`);
...
<code class="shot-ph-path">public/stocks/images/${esc(file)}</code>
</div>
${cap}</figure>`;
}
