...
import { CHAPTERS, TAG_CLASS, chapterBySlug } from "@/lib/futuresCurriculum";
    ...
    <div className="grid gap-10 px-5 sm:px-8 lg:px-12 py-10 xl:grid-cols-[minmax(0,1fr)_220px]">
      ...
      <article className="min-w-0">
        ...
        <nav className="mt-14 grid gap-4 border-t border-border pt-7 sm:grid-cols-2">
          {prev ? (
            <Link href={`/futures/${prev.slug}`} className="obsidian-card ghost-border rounded-xl p-4 hover-lift">
          ...
          {next ? (
            <Link href={`/futures/${next.slug}`} className="obsidian-card ghost-border rounded-xl p-4 hover-lift sm:text-right">
