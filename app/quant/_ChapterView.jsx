...
import { CHAPTERS, TAG_CLASS, chapterBySlug } from "@/lib/quantCurriculum";
    ...
    <div className="grid gap-10 px-5 sm:px-8 lg:px-12 py-10 xl:grid-cols-[minmax(0,1fr)_220px]">
      ...
      <article className="min-w-0">
        ...
        <nav className="mt-14 grid gap-4 border-t border-border pt-7 sm:grid-cols-2">
          {prev ? (
            <Link href={`/quant/${prev.slug}`} className="obsidian-card ghost-border rounded-xl p-4 hover-lift">
          ...
          {next ? (
            <Link href={`/quant/${next.slug}`} className="obsidian-card ghost-border rounded-xl p-4 hover-lift sm:text-right">
            ...
            <Link href="/quant" className="obsidian-card ghost-border rounded-xl p-4 hover-lift sm:text-right">
