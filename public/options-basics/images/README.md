# Options Basics - images

The single, common place for every image used by the **Options Basics**
course (`/options-basics`): nano-banana block/flow diagrams, seaborn statistical
charts, and plotly/payoff charts built on real OpenAlgo data.

Put every image file directly in this folder:

```
public/options-basics/images/
```

A chapter references an image from its markdown (`content/options-basics/md/chNN.md`) with:

```
{{image: payoff-long-call.png | The long call payoff: capped loss, unlimited upside | chart}}
```
...
and re-run `npm run gen:options-basics` to swap the placeholder for the real image.
