...
When the exchange decides whether your option is in-the-money and by how much, it does not use the last tick you happened to see. It uses an official **final settlement price**, a closing value the exchange calculates for the underlying at the end of expiry day. For an index it is the closing value of the index, and for a stock the exchange-defined closing price of the share. Either is itself a weighted average over the closing window, not a single last tick. The practical point for a beginner is that you cannot pick the exact moment of settlement. Your in-the-money option is measured against this one official number, whatever it turns out to be, which is another reason many traders prefer to close a position themselves at a price they can see rather than leave the outcome to the closing calculation.
