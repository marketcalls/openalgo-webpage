...
## Handy methods to know

A Series comes loaded with one-call analysis. The ones you'll use constantly:

- **`.head()` / `.tail()`** - peek at the first or last few values.
- **`.mean()` / `.std()` / `.min()` / `.max()`** - instant statistics.
- **`.pct_change()`** - period-over-period returns.
- **`.cumsum()` / `.cumprod()`** - running totals and compounding.
- **`.describe()`** - a quick statistical summary all at once.
- **`.rolling(20).mean()`** - a moving average (we'll meet rolling windows in Chapter 30).
